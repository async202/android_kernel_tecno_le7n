...
#include "s5kgm1st_mipiraw_sensor.h"
...
#endif
static bool __maybe_unused bNeedSetNormalMode = KAL_FALSE;
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =
{
	...
	{4000, 3000,  0,   0, 4000, 3000, 2000, 1500, 0000, 0000, 2000, 1500, 0, 0, 2000, 1500},  /* hight speed video */
...
static struct SET_PD_BLOCK_INFO_T __maybe_unused imgsensor_pd_info_16_9 =
{
	.i4OffsetX = 16,
	.i4OffsetY = 60,
	.i4PitchX  = 32,
...
static struct SET_PD_BLOCK_INFO_T __maybe_unused imgsensor_pd_info_binning =
{
	.i4OffsetX = 16,
	.i4OffsetY = 60,
	.i4PitchX  = 32,
...
static void set_shutter(kal_uint32 shutter)
{
	...
	spin_lock(&imgsensor_drv_lock);
	if (shutter > imgsensor.min_frame_length - 4)
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			if (*sensor_id == S5KGM1ST_SENSOR_ID || *sensor_id == S5KGM1SP_SENSOR_ID) {
...
{
	sensor_resolution->SensorFullWidth = 4000;
	sensor_resolution->SensorFullHeight = 3000;
	sensor_resolution->SensorPreviewWidth = 4000;
	sensor_resolution->SensorPreviewHeight = 3000;
	sensor_resolution->SensorVideoWidth = 4000;
	sensor_resolution->SensorVideoHeight = 3000;
	sensor_resolution->SensorHighSpeedVideoWidth = 2000;
	sensor_resolution->SensorHighSpeedVideoHeight = 1500;
	sensor_resolution->SensorSlimVideoWidth = 4000;
	sensor_resolution->SensorSlimVideoHeight = 3000;
	sensor_resolution->SensorCustom1Width = 4000;
	sensor_resolution->SensorCustom1Height = 3000;
	return ERROR_NONE;
}	/*	get_resolution	*/
		 ...
		 MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	sensor_info->SensorPreviewResolutionX = imgsensor_info.pre.grabwindow_width;
	sensor_info->SensorPreviewResolutionY = imgsensor_info.pre.grabwindow_height;
	sensor_info->SensorFullResolutionX = imgsensor_info.cap.grabwindow_width;
	sensor_info->SensorFullResolutionY = imgsensor_info.cap.grabwindow_height;
	...
	sensor_info->SensorResetActiveHigh = 0;
	...
	sensor_info->SensorGrabStartX = 0;
	sensor_info->SensorGrabStartY = 0;
	...
	sensor_info->MIPIDataLowPwr2HighSpeedSettleDelayCount = 0x55;
	sensor_info->SensorWidthSampling = 1;
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME:
			if (feature_data && *(feature_data + 1)) {
				spin_lock(&imgsensor_drv_lock);
				imgsensor.frame_length = (kal_uint16)*(feature_data + 1) > 0x0c7a ? (kal_uint16)*(feature_data + 1) : 0x0c7a;
				imgsensor.min_frame_length = imgsensor.frame_length;
				spin_unlock(&imgsensor_drv_lock);
		...
		case SENSOR_FEATURE_CHECK_SENSOR_ID:
		{
			...
			kal_uint32 req_module_id = *feature_data;
			get_imgsensor_id(&sensor_id);
			if (sensor_id == S5KGM1ST_SENSOR_ID || sensor_id == S5KGM1SP_SENSOR_ID) {
				if (req_module_id != 0) {
					kal_uint8 eeprom_module_id = 0;
					kal_uint16 eeprom_reg = 0x0100;
					iReadRegI2C((kal_uint8 *)&eeprom_reg, 2, (kal_uint8 *)&eeprom_module_id, 2, 0xa0);
					if (eeprom_module_id == req_module_id) {
						printk("s5kgm1stmipiraw_Sensor i2c write id: 0x%x, sensor id: 0x%x read_module_id(OK)=%d ++\n",
							imgsensor.i2c_write_id, S5KGM1ST_SENSOR_ID, req_module_id);
						*feature_return_para_32 = S5KGM1ST_SENSOR_ID;
					} else {
						printk("s5kgm1stmipiraw_Sensor i2c write id: 0x%x, read_module_id(Fail)=%d req=%d --\n",
							imgsensor.i2c_write_id, eeprom_module_id, req_module_id);
					...
					kal_uint16 eeprom_reg = 0x0100;
					iReadRegI2C((kal_uint8 *)&eeprom_reg, 2, (kal_uint8 *)&eeprom_module_id, 2, 0xa0);
					printk("s5kgm1stmipiraw_Sensor i2c write id: 0x%x, sensor id: 0x%x read_module_id(read)=%d ++\n",
						imgsensor.i2c_write_id, S5KGM1ST_SENSOR_ID, eeprom_module_id);
					*feature_return_para_32 = S5KGM1ST_SENSOR_ID;
				}
			} else {
				*feature_return_para_32 = 0xFFFFFFFF;
		...
		case SENSOR_FEATURE_GET_CROP_INFO:
			if (feature_data && *(feature_data + 1)) {
				kal_uint32 scen = *feature_data;
				if (scen <= 5) {
					memcpy((void *)(uintptr_t)*(feature_data + 1),
					       &imgsensor_winsize_info[scen],
		...
		case SENSOR_FEATURE_GET_VC_INFO:
			if (feature_data && *(feature_data + 1)) {
				...
				kal_uint32 scen = *feature_data;
				if (scen == 2)
					memcpy(pvcinfo, &SENSOR_VC_INFO[2], sizeof(struct SENSOR_VC_INFO_STRUCT));
				else if (scen == 1)
					memcpy(pvcinfo, &SENSOR_VC_INFO[1], sizeof(struct SENSOR_VC_INFO_STRUCT));
				else
					memcpy(pvcinfo, &SENSOR_VC_INFO[0], sizeof(struct SENSOR_VC_INFO_STRUCT));
			}
			break;
