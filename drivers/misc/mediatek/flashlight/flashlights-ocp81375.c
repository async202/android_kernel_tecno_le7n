...
static struct project_current_config_struct
	g_project_current_config[OCP81375_CHANNEL_NUM] = {
	{
		...
		.torch_360 = 72,
		.phonecall = 22,
		.torch_duty_range = {10, 150},
		.duty_num = 30,
		.app_duty_current = {
			22, 90, 158, 226, 294, 362, 430, 498, 566, 634,
			702, 770, 838, 906, 974, 1042, 1110, 1178, 1246, 1314,
			1382, 1450, 1518, 1586, 1654, 1722, 1790, 1858, 1926, 1994
		},
	},
	{
		.hw_limit = {500, 1000},
...
static void generate_dev_para(int channel, int config_idx)
{
	int i;
	struct project_current_config_struct *cfg = &g_project_current_config[config_idx];
	struct flashlight_dev_para_struct *para = &ocp81375_para[channel];

	para->duty_num = cfg->duty_num;
	para->min_flash_duty = 30;
	para->flashmode = 0;
	para->max_torch_current = 500;
	para->max_flash_current = 2000;

	for (i = 0; i < 30; i++) {
		int regval;

		if (i < cfg->duty_num) {
			int current_ma = cfg->app_duty_current[i];

			if (current_ma > cfg->hw_limit[1])
				current_ma = cfg->hw_limit[1];

			if (current_ma > para->max_torch_current) {
				if (i < para->min_flash_duty)
					para->min_flash_duty = i;
			}

			if (current_ma > para->max_flash_current) {
				regval = 0x7f;
			} else if (current_ma < 4) {
				regval = 0;
			} else if (current_ma <= para->max_torch_current) {
				regval = (current_ma * 100 - 390) / 390;
			} else {
				regval = (current_ma * 100 - 1563) / 1563;
			}
			...
			if (cfg->duty_num > 0)
				regval = para->duty_reg_code[cfg->duty_num - 1];
			else
				regval = 0;
		}

		para->duty_reg_code[i] = regval;
...
static int ocp81375_probe(struct platform_device *dev)
{
	...
	for (i = 0; i < OCP81375_CHANNEL_NUM; i++) {
	...
	generate_dev_para(OCP81375_CHANNEL_CH2, 0);
	generate_dev_para(OCP81375_CHANNEL_CH1, 1);
