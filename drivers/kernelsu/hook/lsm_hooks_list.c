...
static __nocfi int ksu_setprocattr_new2(const char *lsm, const char *name, void *value, size_t size)
{
	assume(!!setprocattr_fn);
	ksu_hide_setprocattr_inline(name, value, size);
	return ((typeof(ksu_setprocattr_new2) *)setprocattr_fn)(lsm, name, value, size);
}
...
(fn),								\
	...
	int (*)(SETPROCATTR_TYPE_new2)	:ksu_setprocattr_new2 	\
)
