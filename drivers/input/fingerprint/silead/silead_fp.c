...
{
	/* Fingerprint scanner is purely a biometric sensor on this device.
	 * Do not report navigation / key events (like Home / Back) to the system.
	 */
	return 0;
}
...
{
	/* Do not register input device - matching stock kernel behavior.
	 * Fingerprint sensor is purely a biometric scanner, not a button.
	 */
	fp_dev->input = NULL;
	return 0;
}
...
static int silfp_probe(struct spi_device *spi)
{
	...
	if (FP_DEV_MAJOR > 0) {
		...
		status = register_chrdev_region(fp_dev->devt, 2, FP_DEV_NAME);
	} else {
		status = alloc_chrdev_region(&fp_dev->devt, fp_dev->ref, 2, FP_DEV_NAME);
	}
	if (status < 0) {
	...
	device_create(silfp_class, &fp_dev->spi->dev, MKDEV(MAJOR(fp_dev->devt), MINOR(fp_dev->devt) + 1), fp_dev, "tran_fp");

	cdev_init(&fp_dev->cdev, &silfp_dev_fops);
	fp_dev->cdev.owner = THIS_MODULE;
	status = cdev_add(&fp_dev->cdev, fp_dev->devt, 2);
	if (status) {
...
err_dev:
	device_destroy(silfp_class, MKDEV(MAJOR(fp_dev->devt), MINOR(fp_dev->devt) + 1));
	if (!IS_ERR_OR_NULL(fp_dev->dev))
		device_destroy(silfp_class, fp_dev->devt);
	unregister_chrdev_region(fp_dev->devt, 2);
...
/*-------------------------------------------------------------------------*/
static struct kobject *tran_fp_kobj;
...
static ssize_t vendor_name_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	return count;
}

static struct kobj_attribute vendor_name_attribute =
	__ATTR(vendor_name, 0664, vendor_name_show, vendor_name_store);
...
int silfp_dev_init(void)
{
	...
	tran_fp_kobj = kobject_create_and_add("tran_fp", kernel_kobj);
	if (tran_fp_kobj) {
		if (sysfs_create_file(tran_fp_kobj, &vendor_name_attribute.attr)) {
			pr_err("Failed to create /sys/kernel/tran_fp/vendor_name node\n");
		}
	}
...
{
	if (tran_fp_kobj) {
		sysfs_remove_file(tran_fp_kobj, &vendor_name_attribute.attr);
		kobject_put(tran_fp_kobj);
		tran_fp_kobj = NULL;
	}
